 ...
 */
const playTone = (whatever = "C4", type = "sine", duration = 1.3, panValue = 0) => {
  if (typeof whatever === "object") {
    playChord(whatever, type, duration);
  } else if (typeof whatever === "string") {
    playSingleFrequency(Tone.getToneFrequency(whatever), type, duration, panValue);
  } else if (typeof whatever === "number") {
    playSingleFrequency(whatever, type, duration, panValue);
  }
};
 ...
 */
const playChord = (chords = 440, type = "sine", duration = 1.3, panValue = 0) => {
  for (let i = 0; i <= 2; i++) {
    playSingleFrequency(Tone.getToneFrequency(chords[i]), type, duration, panValue);
  }
};
